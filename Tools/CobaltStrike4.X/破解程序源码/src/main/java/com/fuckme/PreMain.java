            ...
            try {
                if (className == null) {
                    ...
                    mtd.setBody(
                            ...
                            "}");
                    return cls.toBytecode();
                }else if (className.equals("sun/management/RuntimeImpl")){
                    CtClass cls = classPool.makeClass(new ByteArrayInputStream(classfileBuffer));
                    CtMethod mtd = cls.getDeclaredMethod("getInputArguments");
                    classPool.importPackage("java.util.ArrayList");
                    mtd.insertAfter( "ArrayList ret = new ArrayList();" +
                            "for (int i=0; i < $_.size(); i++) {" +
                            "   if($_.get(i).toString().toLowerCase().contains(\"-javaagent:\")) continue;" +
                            "   ret.add($_.get(i));" +
                            "}" +
                            "$_ = ret;");
                    return cls.toBytecode();
